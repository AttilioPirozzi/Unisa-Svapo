...
<%
	...
	ArrayList<ProductBean>products=(ArrayList<ProductBean>)request.getAttribute("productlist");
%>
  ...
  <div id="content">
    ...
    <section class="mainContent">
      <div class="product_list">
					...
					%>
							<div class="product">
								<a href="pagecomposer?responsepage=prodotto&id=<%=p.getCodiceProdotto() %>">
								<img class="image" src="img/prodotti/<%=p.getCodiceProdotto()%>/prodotto.jpg" alt="Immagine non Disponibile" height="400" width="400" onerror="this.src='notfound.jpg';" /></a>
								<div class="product_proprieties">
									<p class="desc"><i><b>Descrizione: </b></i><%= p.getDescrione() %></p>		
									<p class="price"><i><b>Prezzo: </b></i> <%=p.getPrezzoV() %> euro</p>				
									<p class="iva"><i><b>IVA: </b></i> <%=p.getAliquota() %>%</p>				
									<p class="quant"><i><b>Quantità disponibile: </b></i> <%=p.getQuantita() +" "+p.getUnitaDiMisura()%></p>				
									<p class="category"><i><b>Categoria: </b></i> <%=p.getCategoria() %></p>
								</div>
								<% if(session.getAttribute("user")!=null) {%>
								<form class="add_to_cart_button" method="get" action="addtocart">
									<input type="hidden" value="<%=p.getCodiceProdotto() %>" name="id" >
									<input required="required" type="number" min="1" max="<%= p.getQuantita()%>" name="quantity">

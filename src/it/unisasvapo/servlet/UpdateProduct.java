...
public class UpdateProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String mag=request.getParameter("magazzino");
		float q=Float.parseFloat(request.getParameter("newQuant"));
		if(information.updateProduct(id,mag,q)){
			request.setAttribute("state", "riuscita");
		}else{
			request.setAttribute("state", "non riuscita");
		}
		request.getServletContext().getRequestDispatcher("/pagecomposer?responsepage=after_update").forward(request, response);
